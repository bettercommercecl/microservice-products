...
export default class ChannelsController {
  ...
  /**
   * Sincroniza canales desde la configuracion (utils/channels).
   * Crea o actualiza cada canal con id y nombre segun la config.
   */
  async sync({ response }: HttpContext) {
    ...
    try {
      const toSync: { id: number; name: string }[] = []
      for (const [brand, countries] of Object.entries(channelsConfig)) {
